...
mpu.bin: main.c blink.c startup.c vector_table.s asm_func.s $(RTOS) $(CAMERA)
	$(CROSS-COMPILER)gcc -std=c11 -Wall -mcpu=cortex-m4 -mthumb -mfloat-abi=hard -mfpu=fpv4-sp-d16 -nostartfiles -T stm32f4.ld main.c blink.c startup.c $(RTOS) $(CAMERA) vector_table.s asm_func.s -o mpu.elf

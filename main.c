...
void Distributed_NodeResponseSubtaskFinish(uint32_t Target_Node_id, uint32_t Target_Subtask_id);
...
void Distributed_NodeRemoveTask(uint32_t Target_Node_id, uint32_t Task_id);
...
void UserDefine_Distributed_Task();
...
uint8_t SendFreeBlockFlag = 0;
uint32_t RecvFreeBlockFlag = 0;
...
void Distributed_CreateTask(void* task, Distributed_Data_t* data_info, uint32_t Stack_size){
	TaskHandle_t Now_TaskHandle = xTaskGetCurrentTaskHandle();
	printf("Now_TaskHandle: 0x%lX\r\n", (uint32_t)Now_TaskHandle);
...
void Distributed_LocalSubtaskDone(Distributed_TaskHandle_List_t* s, uint32_t* Result_Data_addr, uint32_t Result_Data_size) {
	...
	if(tmp_NewDTaskControlBlock->Source_Processor_id == Global_Node_id){															//	Check Task done, DStart list without task_id(all subtask done)
		...
		while(check_Lastnode != NULL){
			if(check_Lastnode->DTask_id == tmp_NewDTaskControlBlock->DTask_id)
				tmp_count++;
...
Distributed_TaskHandle_List_t* Distributed_DispatchTask(void* data_info, uint32_t sp, uint32_t lr){
	...
	uint8_t Get_ket = 0;
	while(Get_ket == 0){
		Get_ket = Distributed_NodeRequestReleaseSequence(Request);
	}
	printf("In Distributed_DispatchTask\r\n");
	vPortEnterCritical();
	critial_count++;
	DebugFlag = 1;
	Global_Task_id++;
		...
		if(success_dispatch_flag > 0){																		//	All node could be dispatch
			...
			for(uint32_t split_num_th=0;split_num_th<act_split_num;split_num_th++){
				if(Distributed_dispatch_node[split_num_th] != Global_Node_id){
					/*
					uint32_t BlockChangeFlag_ori = BlockChangeFlag;
					...
					*/
					...
					vPortEnterCritical();
					critial_count++;
					//BlockChangeFlag = BlockChangeFlag_ori;
	...
	if(satisfy_split_num == 0){
		...
		for(uint32_t split_num_th=0;split_num_th<satisfy_split_num;split_num_th++){
			...
			DebugFlag = 29;
			//printf("Data from: 0x%lX, 0x%lX to 0x%lX, 0x%lX\r\n", (uint32_t)dest_data_addr, *dest_data_addr, (uint32_t)(tmp_dest_data_addr-1), *(tmp_dest_data_addr-1));
			...
			if(split_num_th == 0){
				...
				while(Remain_Send_Size > 0){
					...
					//printf("Distributed_NodeSendSubtask node id: 0x%lX, Remain_th: 0x%lX\r\n", Distributed_dispatch_node[split_num_th], Remain_th);
					vPortExitCritical();
					critial_count--;
	...
	Get_ket = 0;
	while(Get_ket == 0)
		Get_ket = Distributed_NodeRequestReleaseSequence(Release);
	DebugFlag = 44;
	printf("Out Distributed_DispatchTask\r\n");
	svc_tick = xTaskGetTickCount();
...
void svc_handler_c(uint32_t LR, uint32_t MSP){
	...
	if(svc_num == 0)
		...
		*(stack_frame_ptr) = (uint32_t)Distributed_GetNode(stacked_return_addr, DStart);
		...
		distributed_task_duration_tick = xTaskGetTickCount()- svc_tick;
			...
			if(SubtaskfinishFlag == 0){
				...
				if(Lastnode != NULL){
					...
					*/
					...
					SubtaskfinishFlag = (uint32_t)Lastnode;
					...
					if(Lastnode != NULL){
						printf("\r\n\r\nSubtask in the DFinish list\r\n\r\n");
						//SubtaskfinishFlag = (uint32_t)Lastnode;
						Distributed_NodeResponseSubtaskFinish(Sour, subtask_id);
						printf("Response Distributed_NodeResponseSubtaskFinish, and the node is not in dstart\r\n");
					}
					else{
						printf("\r\n\r\nNo Subtask Here, Task id: 0x%lX, Subtak id: 0x%lX\r\n\r\n", task_id, subtask_id);
						SubtaskfinishFlag = 0xFFFFFFFF;
						No_Node_ID = Sour;
						No_Task_ID = task_id;
						No_Subtask_ID = subtask_id;
						Distributed_NodeRemoveTask(Global_Node_id, task_id);
						printf("Response Distributed_NodeRemoveTask, and the node is not in dstart\r\n");
					}
		...
		if ((READ_BIT(USART1_BASE + USART_SR_OFFSET, RXNE_BIT)) || (READ_BIT(USART1_BASE + USART_SR_OFFSET, ORE_BIT))){
			...
			if (rec_cmd == 'c'){
				...
				while(1){
					...
					if (rec_cmd == 'f'){										//	send check all node
						...
						for(uint32_t i=0;i<free_block_number;i++){
							...
							if(Allfreeblocknode[i] != Global_Node_id){
								vPortExitCritical();
								critial_count--;
								timeout_flag = Distributed_NodeCheckSizeTimeout(4*timeout_tick_count, Allfreeblocknode[i], 0);
								vPortEnterCritical();
								critial_count++;
					...
					if(ReceiveSubtaskFlag > 0){
						...
						if(tmp_event == Distributed_NodeSendSubtask_MSG){
							...
							if(Distributed_Recv_Size > Recv_Data_Number){
								...
								//printf("Distributed_Recv_Size: 0x%lX, Got Recv_Data_Number: 0x%lX, Remain_Data_number: 0x%lX\r\n", Distributed_Recv_Size, Recv_Data_Number, Remain_Data_number);
								DebugFlag = 512 + 14;
								if(Remain_Data_number > 0){
									while(Remain_Data_number > 0){
										...
										if(Remain_th == 0){
											...
											//printf("Distributed_Recv_Size: 0x%lX, Remain_Data_number: 0x%lX, Remain_th: 0x%lX\r\n", Distributed_Recv_Size, Remain_Data_number, Remain_th);
										}
										else{
											Distributed_NodeRecvRemainSubtask(Source_node, Remain_th);
											//printf("Distributed_Recv_Size: 0x%lX, Remain_Data_number: 0x%lX, Remain_th: 0x%lX\r\n", Distributed_Recv_Size, Remain_Data_number, Remain_th);
										}
										//printf("Distributed_NodeRecvSubtask node id: 0x%lX, Remain_th: 0x%lX\r\n", Source_node, Remain_th);
										DebugFlag = 512 + 15;
										vPortExitCritical();
										critial_count--;
										...
										if((ReceiveSubtaskFlag == NewDTaskControlBlock->Source_Processor_id) && (RemainThFlag == (Remain_th+1))){
											...
											//printf("Receive Distributed_NodeSendRemainSubtask, Remain_th: 0x%lX\r\n", RemainThFlag);
								...
								#endif
								DebugFlag = 512 + 23;
								Distributed_NodeRecvSubtask(ReceiveSubtaskFlag);
							}
							DebugFlag = 512 + 24;
							ReceiveSubtaskFlag = 0;
							#if(PrintSendRecv > 0)
					...
					else if(SubtaskfinishFlag != 0){
						DebugFlag = 512 + 29;
						vPortEnterCritical();
						critial_count++;
						if(SubtaskfinishFlag != 0xFFFFFFFF){
							DebugFlag = 512 + 30;
							Distributed_TaskHandle_List_t* Lastnode = (Distributed_TaskHandle_List_t*)SubtaskfinishFlag;
							if(Lastnode->Source_Processor_id == Global_Node_id){							//	Check Task done, DStart list without Lastnode->DTask_id(all subtask done)
								...
								if(tmp_count == 0){
									...
									//printf("DTask_id: 0x%lX done, DSubTask_id: 0x%lX  is the last===============================================\r\n", Lastnode->DTask_id, subtask_id);
								}
							}
							#if(PrintSendRecv > 0)
								printf("Send Distributed_NodeResponseSubtaskFinish\r\n");
							#endif
							/*
							Distributed_NodeResponseSubtaskFinish(Lastnode->Destinate_Processor_id, Lastnode->DSubTask_id);

							vPortExitCritical();
							critial_count--;
							uint8_t Flag = 0;
							while(Flag == 0){
								Flag = Distributed_NodeRecvCompleteSequence(No_Node_ID);
								if(Flag == 0)
									Distributed_NodeResponseSubtaskFinish(Lastnode->Destinate_Processor_id, Lastnode->DSubTask_id);
							}
							vPortEnterCritical();
							...
							*/
							...
							DebugFlag = 512 + 32;
							/*
							Distributed_NodeResponseSubtaskFinish(No_Node_ID, No_Subtask_ID);
							vPortExitCritical();
							critial_count--;
							uint8_t Flag = 0;
							while(Flag == 0){
						...
						while(Send_Remain_Size > 0){
							...
							if((ConfirmResultFlag == Resultnode->Source_Processor_id) && (RemainThResultFlag == (Remain_th+1))){
								...
								/*
								if((ConfirmResultFlag == Resultnode->Source_Processor_id) && (RemainThResultFlag != (Remain_th+1)))
									printf("Dame Remain_th is wrong, Remain_th here: 0x%lX, Remain_th+1: 0x%lX, RemainThResultFlag: 0x%lX\r\n", Remain_th, (Remain_th+1), RemainThResultFlag);
								else
									printf("Dame ConfirmResultFlag is wrong, ConfirmResultFlag: 0x%lX, Resultnode->Source_Processor_id: 0x%lX\r\n", ConfirmResultFlag, Resultnode->Source_Processor_id);
								*/
								DebugFlag = 512 + 58;
								...
								/*
								if(Remain_th == 0)
									printf("Timeout in Distributed_NodeResponseResult: 0x%lX\r\n", Remain_th);
								else
									printf("Timeout in Distributed_NodeResponseRemainResult: 0x%lX\r\n", Remain_th);
								*/
							}
						}
						DebugFlag = 512 + 59;
						...
						if(Lastnode != NULL){
							...
							while(1){
								...
								//while(!PublishFlag);
								DebugFlag = 512 + 66;
								//printf("Distributed_NodeSubtaskFinish\r\n");
								ResponseSubtaskFinishFlag = 0;
								...
								if(ResponseSubtaskFinishFlag == tmp_NewDTaskControlBlock->DSubTask_id){
									DebugFlag = 512 + 67;
									printf("ResponseSubtaskFinishFlag success, DSubTask_id: 0x%lX\r\n", ResponseSubtaskFinishFlag);
									ResponseSubtaskFinishFlag = 0;
									DebugFlag = 512 + 68;
									/*
									vPortExitCritical();
									...
									uint8_t Flag = 0;
									while(Flag == 0)
										Flag = Distributed_NodeSendCompleteSequence(tmp_NewDTaskControlBlock->Source_Processor_id);
									vPortEnterCritical();
									critial_count++;
									*/
									break;
						...
						DebugFlag = 512 + 73;
						printf("unmerge_finish_distributed_task done\r\n");
						vPortExitCritical();
						critial_count--;
					}
						...
						if(Get_ket != 0){
							...
							if(Lastnode == NULL){
								printf("Fuck can't find target node, TaskDoneFlag: 0x%lX\r\n", TaskDoneFlag);
								Lastnode = DFinish;
								while(Lastnode != NULL){
									printf("Lastnode, Source_Processor_id: 0x%lX, DTask_id: 0x%lX, DSubTask_id: 0x%lX\r\n", Lastnode->Source_Processor_id, Lastnode->DTask_id, Lastnode->DSubTask_id);
									Lastnode = Lastnode->Next_TaskHandle_List;
								}
							}
							printf("TaskDoneFlag: 0x%lX, DTask_id: 0x%lX\r\n", TaskDoneFlag, Lastnode->DTask_id);
							DebugFlag = 512 + 77;
							if(Lastnode == DFinish)
							...
							printf("Targetnodehead, DTask_id: 0x%lX, DSubTask_id: 0x%lX\r\n", Targetnodehead->DTask_id, Targetnodehead->DSubTask_id);
							DebugFlag = 512 + 78;
							if(Lastnode != NULL){
								DebugFlag = 512 + 79;
								...
								printf("Last_target_node, target_node_count: 0x%lX, DTask_id: 0x%lX, DSubTask_id: 0x%lX\r\n", target_node_count, Last_target_node->DTask_id, Last_target_node->DSubTask_id);
								DebugFlag = 512 + 80;
								...
								#if (PrintSendRecv > 0)
									printf("Send_S->xQueue: 0x%lX\r\n", (uint32_t)Subtask_node_zero->xQueue);
								#endif
								DebugFlag = 512 + 82;
								...
								for(uint32_t i=0;i<target_node_count;i++){
									...
									if(target_node_array[i]->DSubTask_id == 0){
										...
										while(Recv_Remain_Size > 0){
											...
											DebugFlag = 768 + target_node_array[i]->Destinate_Processor_id;
											if(Remain_th == 1)
											...
											vPortExitCritical();
											critial_count--;
											WaitForFlag(&RequestResultFlag, 10);
											vPortEnterCritical();
											critial_count++;
											DebugFlag = 512 + 91;
											if((RequestResultFlag == target_node_array[i]->Destinate_Processor_id) && (RemainThResultFlag == Remain_th)){
												...
												/*
												if((RequestResultFlag == target_node_array[i]->Destinate_Processor_id) && (RemainThResultFlag != Remain_th))
													printf("Wrong RemainThResultFlag: 0x%lX, Remain_th: 0x%lX\r\n", RemainThResultFlag, Remain_th);
												else
													printf("Wrong RequestResultFlag: 0x%lX, target_node_array[i]->Destinate_Processor_id: 0x%lX\r\n", RequestResultFlag, target_node_array[i]->Destinate_Processor_id);
												*/
										...
										/*
										Distributed_NodeRequestRemainResult(target_node_array[i]->Destinate_Processor_id, Remain_th);
										printf("Distributed_NodeRequestRemainResult, Remain_th: 0x%lX\r\n", Remain_th);
										*/
										Distributed_NodeRequestRemainResult(target_node_array[i]->Destinate_Processor_id, Remain_th);
										vPortExitCritical();
										critial_count--;
									...
									#if (PrintSendRecv > 0)
										Result_Stop_Addr = tmp_Target_Addr;
										for(uint32_t i=0;i<10;i++)
											printf("0x%lX, 0x%lX	0x%lX, 0x%lX\r\n", (uint32_t)(Result_Start_Addr+i), *(Result_Start_Addr+i), (uint32_t)(Result_Stop_Addr-10+i), *(Result_Stop_Addr-10+i));
									#endif
									DebugFlag = 512 + 96;
								}
								DebugFlag = 512 + 97;
								...
								while(Lastnode != NULL){
									Lastnode = Lastnode->Next_TaskHandle_List;
									printf("Lastnode DTask_id: 0x%lX, Subtask_id: 0x%lX\r\n", Lastnode->DTask_id, Lastnode->DSubTask_id);
								}
								Lastnode = Targetnodehead;
								DebugFlag = 512 + 99;
								Distributed_NodeRemoveTask(Global_Node_id, Lastnode->DTask_id);
								while(Lastnode != NULL){
								...
								vPortExitCritical();
								critial_count--;
								printf("xQueueSendToBack, critial_count: 0x%lX\r\n", critial_count);
								xQueueSendToBack(*((QueueHandle_t*)Send_S->xQueue), (void*)&Send_S, 0);
								vTaskResume(Send_S->TaskHandle);
								printf("Send_S->TaskHandle: 0x%lX\r\n", (uint32_t)Send_S->TaskHandle);
								vPortEnterCritical();
								critial_count++;
								DebugFlag = 512 + 101;
								printf("TaskDoneFlag\r\n");
...
void Distributed_NodeResponseSubtaskFinish(uint32_t Target_Node_id, uint32_t Target_Subtask_id){
	...
	uint8_t MyMacAddr[6] = {0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
	uint8_t mydata[17] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, Distributed_NodeResponseSubtaskFinish_MSG, 0x00, 0x00, 0x00, 0x00};
	for(uint8_t i=0;i<4;i++){
		MyMacAddr[2+i] = *((uint8_t*)&Global_Node_id+i);
		mydata[i+2] = *((uint8_t*)&Target_Node_id+i);
		mydata[i+8] = *((uint8_t*)&Global_Node_id+i);
...
void Distributed_NodeRemoveTask(uint32_t Target_Node_id, uint32_t Task_id){
	...
	Distributed_SendMsg(MyMacAddr, mydata, 21);
	#if(PrintSendRecv > 0)
		printf("Broadcast Distributed_NodeRemoveTask, Target_Node_id: 0x%lX, Task_id: 0x%lX\r\n", Target_Node_id, Task_id);
	#endif
}
...
uint8_t Distributed_NodeRequestReleaseSequence(uint8_t DisableEnableFlag){
	uint32_t record_tick = xTaskGetTickCount();
	while(1){
		if(Global_Node_id == Global_Node_Master){									//	Local node is Master
			...
			while(ResponseKeyFlag == 0){
				...
				if(DisableEnableFlag == 0){
					//printf("Distributed_NodeRequestKey to Master\r\n");
					Distributed_NodeRequestKey();								//	Request by communication
				}
				else{
					//printf("Distributed_NodeReleaseKey to Master\r\n");
					Distributed_NodeReleaseKey();								//	Release by communication
				}
				WaitForFlag(&ResponseKeyFlag, 10);
				if(ResponseKeyFlag == 0){
					if(DisableEnableFlag == 0)
						;
						//printf("Timeout in Distributed_NodeRequestKey\r\n");
					else
						;
						//printf("Timeout in Distributed_NodeReleaseKey\r\n");
				}
			}
		}
	...
	for(uint32_t i=1;i<=Global_Node_count;i++){									//	Disable/Enable all node
		if((i != Global_Node_Master) && (i != Global_Node_id)){
			uint32_t Timeout_count_limit = 100;
			uint32_t Timeout_count = 0;
			while(PublishResponseFlag != i){
				while(!Check_Sendable_Without_Limit());
				if(DisableEnableFlag == 0){
					//printf("Distributed_NodeDisablePublish to Node: 0x%lX\r\n", i);
					Distributed_NodeDisablePublish(i);
				}
				else{
					//printf("Distributed_NodeEnablePublish to Node: 0x%lX\r\n", i);
	...
	if(SendCompleteFlag > Remain_th){
		...
		while(1){
			...
			if(SendCompleteFlag > Remain_th){
				...
				send_complete_count++;
				if(send_complete_count > send_complete_limit){
					#if(PrintSendRecv > 0)
						printf("NodeSendComplete Not Success, but the last package is sent\r\n");
					#endif
					break;
				}
			}
	...
	if(SendCompleteFlag > Remain_th){
		...
		while(1){
			...
			if(SendCompleteFlag > Remain_th){
				...
				complete_Remain_th = Remain_th;
				SendCompleteFlag = 0;
				recv_complete_count = 0;
				recv_complete_limit = 5;
				while(1){
					//printf("Recv: 0x%lX\r\n", (uint32_t)Remain_th);
					Distributed_NodeSendComplete(Target_Node_id, Remain_th);
					vPortExitCritical();
					...
					if(SendCompleteFlag > Remain_th){
						...
						break;
						#if(PrintSendRecv > 0)
							printf("NodeRecvComplete SUCCESS\r\n");
						#endif
					}
					else{
						recv_complete_count++;
						if(recv_complete_count > recv_complete_limit){
							#if(PrintSendRecv > 0)
								printf("NodeRecvComplete Not Success, but the last package is received\r\n");
							#endif
							break;
						}
					}

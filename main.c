	...
	if(svc_num == 0)
		...
		*(stack_frame_ptr) = (uint32_t)Distributed_GetNode(stacked_return_addr, DStart);
		...
		if(Lastnode->Source_Processor_id == Global_Node_id){
																				//	Check Task done, DStart list without task_id(all subtask done)
				...
				while(1){
					...
					if(ReceiveSubtaskFlag > 0){
						...
						while(Send_Remain_Size > 0){
							...
							vPortExitCritical();
							//ebugFlag = 512 + 56;
							DebugFlag = 702 + Resultnode->Source_Processor_id;
						...
						if(Get_ket != 0){
							...
							if(Lastnode == NULL){
								printf("Fuck can't find target node\r\n");
								Lastnode = DFinish;
								while(Lastnode != NULL){
								...
								for(uint32_t i=0;i<target_node_count;i++){
									...
									if(target_node_array[i]->DSubTask_id == 0){
										...
										while(Recv_Remain_Size > 0){
											...
											DebugFlag = 702 + target_node_array[i]->Destinate_Processor_id;
											if(Remain_th == 1)
